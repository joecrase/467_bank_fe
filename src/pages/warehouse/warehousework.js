...
function WareWorkStation() {
    ...
    function getAllOrders() {
        axios.get('http://localhost:8080/order/all').then(function (response) {
        ...
        return allOrders.map(function (item, i) {
            return (
                <OrderTableRow
                    id={i}
                    orderID={item.id}
                    orderDate={item.datePurchased}
                    distinctProducts={item.cart.length}
                    totalProducts={sumItemTotal(item.cart)}
        ...
        <div>
            ...
            <div className='workerTable'>
                <div className='tableRowTitles'>
                    <div className='tableCell'>OrderID</div>
                    <div className='tableCell'>Order Date</div>
                    <div className='tableCell'>Num of Distinct Products</div>
                    <div className='tableCell'>Quantity of Products</div>
                    <div className='tableCell'>Order Status</div>
                </div>
                {renderOrderRows()}
            </div>
        </div>
