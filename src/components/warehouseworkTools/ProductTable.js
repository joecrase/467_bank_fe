...
function ProductTable(props) {
    ...
    function updateOrderStatus(status) {
        ...
        console.log(
            'You wish to change orderid ' + props.orderID + ' to ' + status
        );
    ...
    function renderProductRows() {
        ...
        return props.cart.map(function (item, i) {
            //TODO Ask Josph to put product name with the response with the order
            return (
                <ProductRow
                    productID={item['part'].number}
                    prodName={item['part'].description}
                    quantity={item.amount}
                    fillorder={props.fillorder}
                    checkmarkProduct={checkIfAllProductFullfiled}
                />
            );
        });
    }
        ...
        <div className={'tableRow ' + (props.show ? '' : 'hidden')}>
            <div className='newTable'>
                <div className='tableRowTitles'>
                    <div className='tableCell'>ProductID</div>
                    <div className='tableCell'>Product Description</div>
            ...
            <Link
                ...
                <button
                    onClick={() => updateOrderStatus('filling')}
                    className={
                        'fillOrderButton ' + (props.fillorder ? 'hidden' : '')
                    }>
                    Begin Filling
                </button>{' '}
                {/* TODO 1. fix buttons so they dont extend so much. 2. add another button that will direct to shipping label if authorized */}
            </Link>
            <Link
                to={{
                ...
                <button
                    ...
                    disabled={!allProductsFullfiled}
                    className={
                        'fillOrderButton ' +
                        (props.fillorder ? '' : 'hidden ') +
                        (allProductsFullfiled ? 'green' : 'red')
                    }>
